#!/usr/bin/env python3
"""
Delete all existing NIFTY 50 data and import fresh data from CSV.
"""
...
def process_csv(csv_path: Path) -> List[Dict[str, Any]]:
    ...
    #   2014-02-06,6028.35,6048.35,5965.4,6036.3,0.23,185500
...
async def delete_and_import(csv_path: Path):
    """Delete all NIFTY 50 data and import fresh from CSV."""
    ...
    # Step 1: Delete all NIFTY 50 data
    print("\n🗑️  Deleting all existing NIFTY 50 data...")
    delete_result = await collection.delete_many({"name": "NIFTY 50"})
    print(f"   Deleted {delete_result.deleted_count} NIFTY 50 documents")
    ...
    total = await collection.count_documents({"name": "NIFTY 50"})
    print(f"\n🏁 Import complete!")
    print(f"   Inserted: {inserted}")
    print(f"   Total NIFTY 50 documents in MongoDB: {total}")
    
    # Show date range
    if total > 0:
        first = await collection.find_one({"name": "NIFTY 50"}, sort=[("date", 1)])
        last = await collection.find_one({"name": "NIFTY 50"}, sort=[("date", -1)])
        if first and last:
...
async def main():
    csv_path = Path("/Users/saikiran/Desktop/aa/nifty50_historical_data_2010_to_today.csv")
